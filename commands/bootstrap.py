...
class WorkflowExecutor:
        ...
        for i, step in enumerate(steps, 1):
            ...
            try:
                if step_type == 'install_application':
                    ...
                    success = await self._execute_execute_step(step)
                else:
    ...
    async def _execute_execute_step(self, step: Dict[str, Any]) -> bool:
        """Execute a contract execution step."""
        node_name = step['node']
        context_id = self._resolve_dynamic_value(step['context_id'])
        exec_type = step.get('exec_type')  # Get exec_type if specified, otherwise will default to function_call
        ...
        original_context_id = step['context_id']
        if '{{context.' in original_context_id and '}}' in original_context_id:
            ...
            if context_key in self.workflow_results:
                context_data = self.workflow_results[context_key]
                ...
                console.print(f"[blue]Debug: Available keys: {list(self.workflow_results.keys())}[/blue]")
