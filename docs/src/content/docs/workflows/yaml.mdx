...
| `await_self` | no | bool | Return only once the new device has folded what the holder wrote. Needs `account_namespace`. |
| `retries` | no | int | Listings `await_self` reads before failing (default 45). |
| `interval` | no | number | Seconds between those reads (default 2). |
