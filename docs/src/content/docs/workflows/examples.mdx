...
| [`account-identity.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/account-identity.yml) | The account plane end to end: enrol an account, pair a SECOND device onto it, assert one account holds two distinct devices, revoke one, then stop the node and export/restore its account root. **Not yet run in CI** — the export/import steps need a merod carrying `merod account export|import` (calimero-network/core#3362). Shows why enrolment must follow the context join (the link op is encrypted under the scope key) and why the backup is offline (RocksDB's lock is exclusive). | `account_create`, `account_pair`, `account_show`, `account_revoke`, `node_exec`, `stop_node`, `start_node`, `json_assert` |
| [`workflow-account-namespace-pairing-example.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/workflow-account-namespace-pairing-example.yml) | Pair a device into the account namespace alone with `account_namespace: auto` and `await_self`, and assert the two `400`s pair-complete owes a tampered offer. | `node_identity`, `account_pair_init`, `account_pair_complete`, `account_pair`, `account_devices`, `assert_api_response` |
| [`workflow-account-rescope-example.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/workflow-account-rescope-example.yml) | `account_rescope` REPLACES a paired device's application scope: narrow it to one application and assert the other namespace's binding is gone, then widen back to `all` and assert it is re-linked, plus the `404` an unknown device owes and the `403` a call off the account holder owes. **Not yet run in CI** - it needs a merod carrying `PUT /admin-api/account/devices/{device_id}/scope` (calimero-network/core#3977); the Docker lane probes the image for that route and starts running the scenario once it is there. | `account_pair`, `account_rescope`, `assert_api_response`, `json_assert` |
...
| [`workflow-assert-api-response-example.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/workflow-assert-api-response-example.yml) | Assertions on the body a node actually sent: `match`, `present`, `absent`, and the negatives `not_contains` and `expect_no_match`, on `assert_api_response` and `account_devices`. | `assert_api_response`, `account_devices`, `node_identity` |
...
| [`workflow-channel-leave-rejoin-example.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/workflow-channel-leave-rejoin-example.yml) | The chat-app channel cycle: self-join a born-Open subgroup by inheritance, leave the context, rejoin it, twice — asserting the leave takes effect, the rejoin restores away-window and pre-leave history, and the rejoiner can author again. A second channel runs as a control. | `join_subgroup_inheritance`, `join_context`, `leave_context`, `wait_for_sync`, `json_assert` |
| [`workflow-namespace-context-leave-rejoin-example.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/workflow-namespace-context-leave-rejoin-example.yml) | The same round trip for a context attached to the namespace ROOT, which rejoins as a direct member and skips the inherited key pull. Also pins that the opt-out is one context wide: the leaver keeps using a subgroup context and joins one created after it left. | `create_context` (on a namespace), `leave_context`, `join_context`, `json_assert` |
| [`workflow-namespace-name-example.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/workflow-namespace-name-example.yml) | `create_namespace`'s `namespace_name`: the creator's own `/admin-api/namespaces` row carries the name, which it does only if the step forwarded it, and a member that joins by invitation reads the same name back. Docker lane only - a released merod stores the name locally but does not publish it to members, so the second assertion needs `merod:edge`. | `create_namespace`, `create_namespace_invitation`, `join_namespace`, `assert_api_response` |
...
| [`workflow-node-exec-example.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/workflow-node-exec-example.yml) | Offline `merod` commands against a stopped node in binary and Docker mode alike: export the recovery phrase, then hand it to `account revoke-proof` as a file under `/app/data`. | `stop_node`, `node_exec`, `assert`, `start_node`, `assert_api_response` |
