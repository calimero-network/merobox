...
| [`account-identity.yml`](https://github.com/calimero-network/merobox/blob/master/workflow-examples/account-identity.yml) | The account plane end to end: enrol an account, pair a SECOND device onto it, assert one account holds two distinct devices, revoke one, then stop the node and export/restore its account root. Shows why enrolment must follow the context join (the link op is encrypted under the scope key) and why the backup is offline (RocksDB's lock is exclusive). | `account_create`, `account_pair`, `account_show`, `account_revoke`, `node_exec`, `stop_node`, `start_node`, `json_assert` |
