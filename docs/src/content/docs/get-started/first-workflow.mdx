...
A context runs a Wasm application packaged as a signed `.mpk` bundle, so the
first step installs one. We use the `kv-store` app (a simple key/value store),
built into `workflow-examples/res/` by running
`workflow-examples/scripts/build_res_bundles.sh` once from the repo root —
`res/` holds no committed artifacts, so this script is what puts the bundle
there.
