...
<Aside type="note">
  ...
  default Docker image is `ghcr.io/calimero-network/merod:prerelease` — an alias
  ghcr repoints at each core prerelease, so a run follows core — and the
  default ports are P2P `2428` and RPC `2528`. Pass `--image` to pin a run to
  a release tag.
</Aside>
...
| `--image`                  | `merod:prerelease` | Custom Docker image (e.g. a release tag, to pin the run).              |
