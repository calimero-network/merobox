...
try:
    from merobox.testing import DockerManager
...
def demo_merobox_installation():
    ...
    if MEROBOX_AVAILABLE:
        ...
        print(
            f"   Version: {getattr(sys.modules.get('merobox'), '__version__', 'Unknown')}"
        )
        print(
            f"   Author: {getattr(sys.modules.get('merobox'), '__author__', 'Unknown')}"
        )
    else:
...
def demo_calimero_manager():
    """Demonstrate the DockerManager functionality."""
    if not MEROBOX_AVAILABLE:
        print("⚠️  Skipping DockerManager demo - Merobox not available")
        return

    print("=== DockerManager Demo ===")
...
def demo_project_structure():
    ...
    print("  📁 src/hello_world/     - Main package")
    ...
    if MEROBOX_AVAILABLE:
        print("  📦 merobox/            - Merobox package (installed)")
