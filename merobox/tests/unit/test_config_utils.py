...
def test_apply_e2e_defaults_binary_mode():
    """Test apply_e2e_defaults without docker_host_url_fn (binary mode)."""
    initial_config = {}
    ...
    with patch("merobox.commands.config_utils.toml") as mock_toml:
        ...
        with patch("builtins.open", mock_open()):
            with (
                ...
                # Verify e2e defaults are applied
                ...
                # Verify binary mode URLs (localhost)
                eth_rpc = config_dict["context"]["config"]["signer"]["self"][
                    "ethereum"
                ]["local"]["rpc_url"]
                assert "127.0.0.1" in eth_rpc


def test_apply_e2e_defaults_docker_mode():
    """Test apply_e2e_defaults with docker_host_url_fn (Docker mode)."""
    initial_config = {}

    def mock_docker_host_url(port):
        ...
        with patch("builtins.open", mock_open()):
            with (
                ...
                result = apply_e2e_defaults(
                    ...
                    docker_host_url_fn=mock_docker_host_url,
                )
                ...
                # Verify Docker mode URLs (host.docker.internal)
                eth_rpc = config_dict["context"]["config"]["signer"]["self"][
                    "ethereum"
                ]["local"]["rpc_url"]
                assert "host.docker.internal" in eth_rpc

                icp_rpc = config_dict["context"]["config"]["signer"]["self"]["icp"][
                    "local"
                ]["rpc_url"]
                assert "host.docker.internal" in icp_rpc
