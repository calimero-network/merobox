...
from merobox.commands.managers.mock_relayer import MockRelayerManager
...
class NodeManager(BaseManager):
    ...
    def __init__(self, client: Optional[docker.DockerClient] = None):
        ...
        self.mock_relayer_manager = MockRelayerManager(self.client)
    ...
    def get_node_rpc_port(self, node_name: str) -> Optional[int]:
        ...
        Returns:
            The RPC port number, or None if not found.
        """
    ...
    def run_node(
        ...
        Args:
            node_name: Name for the node container.
            port: P2P port (default 2428).
            rpc_port: RPC/Admin port (default 2528).
            chain_id: Chain ID (default "testnet-1").
            data_dir: Data directory path.
            image: Docker image to use.
            auth_service: Enable auth service integration.
            auth_image: Custom auth service image.
            auth_use_cached: Use cached auth frontend.
            webui_use_cached: Use cached WebUI frontend.
            log_level: Rust log level.
            rust_backtrace: Rust backtrace setting.
            mock_relayer: Enable mock relayer.
            workflow_id: Workflow ID for test isolation.
            e2e_mode: Enable e2e-style defaults.
            config_path: Custom config.toml path.
            near_devnet_config: NEAR devnet configuration.
            bootstrap_nodes: Bootstrap nodes to connect to.
            use_image_entrypoint: Preserve Docker image's entrypoint.

        Returns:
            True if node started successfully, False otherwise.
        """
        try:
            ...
            relayer_url = None
            if mock_relayer:
                relayer_url = self.mock_relayer_manager.ensure_mock_relayer()
                if not relayer_url:
                    console.print(
    ...
    def run_multiple_nodes(
        ...
        Args:
            ...
            base_port: Base P2P port (auto-assigned if None).
            base_rpc_port: Base RPC port (auto-assigned if None).
            chain_id: Chain ID for all nodes.
            ...
            workflow_id: Workflow ID for test isolation.
            e2e_mode: Enable e2e-style defaults.
            near_devnet_config: NEAR devnet configuration per node.
            bootstrap_nodes: Bootstrap nodes to connect to.
            use_image_entrypoint: Preserve Docker image's entrypoint.
    ...
    def stop_all_nodes(self) -> bool:
        ...
        Returns:
            True if all nodes stopped successfully, False otherwise.
        """
        try:
            ...
            if not self.mock_relayer_manager.stop_mock_relayer():
                success = False
    ...
    def verify_admin_binding(self, node_name: str) -> bool:
        ...
        Returns:
            True if admin server is accessible, False otherwise.
        """
        try:
            if node_name in self.nodes:
                container = self.nodes[node_name]
