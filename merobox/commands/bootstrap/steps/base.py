...
class BaseStep:
        ...
        if '{{' in value and '}}' in value:
            ...
            if value.startswith('{{') and value.endswith('}}'):
                ...
                while True:
                    ...
                    placeholder = result[placeholder_start + 2:placeholder_end].strip()
                    
                    # Resolve the placeholder
                    resolved_value = self._resolve_single_placeholder(placeholder, workflow_results, dynamic_values)
                    
                    # Replace the placeholder in the result string
                    result = result[:placeholder_start] + str(resolved_value) + result[placeholder_end + 2:]
    ...
    def _resolve_single_placeholder(self, placeholder: str, workflow_results: Dict[str, Any], dynamic_values: Dict[str, Any]) -> str:
        ...
        if placeholder.startswith('install.'):
            ...
            if len(parts) == 2:
                ...
                if install_key in workflow_results:
                    ...
                    # Try to extract application ID from the result
                    if isinstance(result, dict):
                        return result.get('id', result.get('applicationId', result.get('name', placeholder)))
                    return str(result)
                else:
                    console.print(f"[yellow]Warning: Install result for {node_name} not found, using placeholder[/yellow]")
                    return placeholder
                ...
                if field_name:
                    ...
                    if context_key in workflow_results:
                        ...
                        if isinstance(result, dict):
                            # Handle nested data structure
                            actual_data = result.get('data', result)
                            return actual_data.get('id', actual_data.get('contextId', actual_data.get('name', placeholder)))
                        return str(result)
                    else:
                        console.print(f"[yellow]Warning: Context result for {node_name} not found, using placeholder[/yellow]")
                        return placeholder
            ...
            if len(parts) == 2:
                ...
                if identity_key in workflow_results:
                    ...
                    if isinstance(result, dict):
                        # Handle nested data structure
                        actual_data = result.get('data', result)
                        return actual_data.get('publicKey', actual_data.get('id', actual_data.get('name', placeholder)))
                    return str(result)
                else:
                    console.print(f"[yellow]Warning: Identity result for {node_name} not found, using placeholder[/yellow]")
                    return placeholder
                ...
                if '_identity.' in invite_part:
                    ...
                    if invite_key in workflow_results:
                        ...
                        if isinstance(result, dict):
                            # Handle nested data structure
                            actual_data = result.get('data', result)
                            return actual_data.get('invitation', actual_data.get('id', actual_data.get('name', placeholder)))
                        return str(result)
                    else:
                        console.print(f"[yellow]Warning: Invite result for {invite_key} not found, using placeholder[/yellow]")
                        return placeholder
