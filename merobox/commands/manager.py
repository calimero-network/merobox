...
class DockerManager:
    ...
    def _cleanup_resources(self):
        """Stop all managed resources (containers and mock relayer)."""
        if self.nodes:
        ...
        # Stop mock relayer if running
        if self.mock_relayer_url:
            try:
                relayer = self.client.containers.get(MOCK_RELAYER_NAME)
                relayer.stop(timeout=10)
                relayer.remove()
                console.print("[green]✓ Stopped mock relayer[/green]")
                self.mock_relayer_url = None
            except docker.errors.NotFound:
                pass
            except Exception as e:
                console.print(f"[yellow]⚠️  Could not stop mock relayer: {e}[/yellow]")
