...
- MockRelayerManager: Handles mock relayer container
...
from merobox.commands.managers import (
    NodeManager,
)
...
class DockerManager:
    ...
    - MockRelayerManager: Mock relayer management
    ...
    def __init__(self):
        """Initialize the DockerManager with all sub-managers."""
        ...
        self.mock_relayer_manager = self._node_manager.mock_relayer_manager
    ...
    @property
    def mock_relayer_url(self) -> Optional[str]:
        """URL of the mock relayer if running."""
        return self._node_manager.mock_relayer_manager.mock_relayer_url

    @mock_relayer_url.setter
    def mock_relayer_url(self, value: Optional[str]):
        """Set the mock relayer URL."""
        self._node_manager.mock_relayer_manager.mock_relayer_url = value
    ...
    def _apply_bootstrap_nodes(
        ...
        return self._node_manager._apply_bootstrap_nodes(
            config_file, node_name, bootstrap_nodes
        )

    def _get_docker_host_url(self, port: int) -> str:
        """Get Docker host URL for a given port."""
        return self._node_manager._get_docker_host_url(port)
    ...
    def _apply_e2e_defaults(
        ...
        return self._node_manager._apply_e2e_defaults(
            config_file, node_name, workflow_id
        )

    def _set_nested_config(self, config: dict, key: str, value):
        """Set nested configuration value using dot notation."""
        return self._node_manager._set_nested_config(config, key, value)
    ...
    def _apply_near_devnet_config(
        ...
        return self._node_manager._apply_near_devnet_config(
            config_file,
            node_name,
            rpc_url,
            contract_id,
